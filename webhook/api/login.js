...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const r = await fetch(SUPA_URL + '/rest/v1/app_users?username=eq.' + encodeURIComponent(username) + '&select=id,username,password_hash,password_salt,empresa_id', {
    ...
    return res.status(200).json({ access_token: access_token, expires_at: exp * 1000, username: user.username, empresa_id: user.empresa_id });
  } catch (e) {
